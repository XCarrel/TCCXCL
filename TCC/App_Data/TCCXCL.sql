...
INSERT INTO config (keyname,charval) VALUES ('defaultGuestName','Invité');
...
-- Associate members to a NPA

INSERT INTO tccmembership (fkuser, NPA) select UserId, 1000 from users;
declare @i int = 1;
while (@i < 200)
begin
	UPDATE tccmembership set NPA =  (select top 1 NPAVal from NPA order by newid()) where idtccmembership=@i;
	set @i = @i+1
end
...
CREATE FUNCTION MembersFrom (@npa int)
RETURNS int
AS
Begin
	return (select count(*) from tccmembership inner join NPA on NPA = NPAVal Where NPA = @npa);
End

GO

CREATE TRIGGER GoodGuest
ON booking
INSTEAD OF INSERT
AS
BEGIN
	...
	Declare cursins Cursor For
	Select moment, fkMadeBy, fkPartner, guest, fkCourt From inserted;

	Open cursins;
	Fetch Next From cursins Into @mom, @uid, @pid, @guest, @cid;

	while @@FETCH_STATUS = 0
	Begin
		if @uid is not null and @pid is null and @guest is null
			set @guest = (SELECT charval FROM config WHERE keyname = 'defaultGuestName');

		Insert Into booking (moment, fkMadeBy, fkPartner, guest, fkCourt) Values (@mom, @uid,@pid,@guest,@cid)
		-- Move on to next record
		Fetch Next From cursins Into @mom, @uid, @pid, @guest, @cid;
	End
...
CREATE TRIGGER grantrole
ON belongs
AFTER INSERT
AS
BEGIN
	declare @sql nvarchar(500) = 'CREATE LOGIN ';
	set @sql = @sql + (select username from inserted inner join Users on fkMember = UserId)
	set @sql = @sql + ' WITH PASSWORD = ''change-me'' MUST_CHANGE, DEFAULT_DATABASE = TCCXCL, CHECK_POLICY = ON, CHECK_EXPIRATION = ON'
	Exec sp_executesql @sql
